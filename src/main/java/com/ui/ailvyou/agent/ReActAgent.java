...
public abstract class ReActAgent extends BaseAgent {
    ...
    public String step() {
        try {
            ...
            if (!shouldAct) {
                // 如果不需要执行动作，说明任务可能已经完成（纯文本回复）
                setState(com.ui.ailvyou.agent.model.AgentState.COMPLETED);
                return getLatestMessageContent();
            }
            return act();
        } catch (BusinessException e) {
            log.error("Agent [{}] 单步执行业务异常: {}", getName(), e.getMessage());
            throw e;
        } catch (Exception e) {
            log.error("Agent [{}] 单步执行系统异常: ", getName(), e);
            throw new BusinessException(ErrorCode.AGENT_STEP_ERROR, "单步执行失败: " + e.getMessage());
        }
    }
}
